  ...
  if [ -z "$JAVA_BIN" ]; then
	...
	case $ANSWER_JAVA in
		...
		if [ -w ~/.bashrc ]; then
		    ...
		    echo "Fallo."
		    echo "Debes reconfigar your aLiteral"
		    exit 1
		fi
		;;
	    ...
	    *)  echo "Debes teclar s  o n"
		echo "Debes reconfigurar aLiteral"
		exit 1
	esac
	export JAVA_BIN
    else
...
$JAVA_BIN/java ${jvm.conf} -Des.uv.aLiteral.${env} -cp $CLASSPATH $DEBUG_OPTIONS  es.uv.aLiteral.main.Bootstrap $*
